...
class MAIN:
 ...
 13. Crack ID Dari File  #===MOD''',True)
       self.chs(self.aset)
   ...
   def chs(self, assets):
       while True:
         ...
         elif x in ['12']:self.Facebook(cookies=self.cokie, token=self.token)
         elif x in ['13']:self.crack_from_file(assets)  #===MOD
...
#MAIN().CariNama()
    ...
    #===MOD Tambahan fitur Crack dari File
    def crack_from_file(self, assets):
        try:
            file_path = Console(style=Tema[0]).input(f'   └──> Masukan nama file target (contoh: data/target.txt) : ')
            with open(file_path, 'r') as f:
                lines = f.read().splitlines()
            for line in lines:
                if '|' in line:
                    self.pk_idg.append(line.strip())
                else:
                    self.pk_idg.append(f'{line.strip()}| ')
            if len(self.pk_idg) == 0:
            ...
            print(f'\nError: {e}')
            exit()
        self.ToolsType()  # langsung ke pilih metode crack
